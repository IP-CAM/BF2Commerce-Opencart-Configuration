...
class ControllerExtensionModuleLetMeKnow extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			*/
			if (!isset($this->request->get['module_id'])) {
				...
				*/
				$this->model_extension_module->addModule('letmeknow', $this->request->post);
			} else {
				/*
		...
		*/
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/letmeknow', 'token=' . $this->session->data['token'], true)
			);
		} else {
			//editar modulo
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/letmeknow', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}
		...
		*/
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['action'] = $this->url->link('extension/module/letmeknow', 'token=' . $this->session->data['token'], true);
		} else {
			//editar o modulo pois vai ter o module_id
			$data['action'] = $this->url->link('extension/module/letmeknow', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/letmeknow')) {
