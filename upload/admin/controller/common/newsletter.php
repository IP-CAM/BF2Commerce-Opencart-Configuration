...
class ControllerCommonNewsletter extends Controller {
	...
	protected function getList() {
		...
		$data['export'] = $this->url->link('common/newsletter/export', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function export($getarr='') {
		...
		$customers_column = array('ID', 'Nome', 'Email','Status');
			
		$customers[0] = $customers_column;  
		
		$customers_list = $this->model_common_newsletter->getNewsletter($data);
		...
		$xls = new Excel_XML('UTF-8', false, 'Newsletter List');
		$xls->addArray($customers);
		$xls->generateXML('newsletterlist_'.date('Y-m-d _ H:i:s'));
	}
